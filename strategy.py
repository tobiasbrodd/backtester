            ...
            for symbol in self.symbol_list:
                ...
                if data is not None and len(data) > 0:
                    if self.bought[symbol] == False:
                        quantity = self.portfolio.current_holdings['cash'] / self.data.get_latest_data(symbol, N=1)[0][6]
        ...
        if event.type == 'MARKET':
            for symbol in self.symbol_list:
                ...
                if data is not None and len(data) >= self.long_period:
                    price_short = df['close'].ewm(self.short_period).mean()[-1]
                    price_long = df['close'].ewm(self.long_period).mean()[-1]
                    if self.bought[symbol] == False and price_short > price_long:
                        quantity = self.portfolio.current_holdings['cash'] / data[0][6]
            ...
            for symbol in self.symbol_list:
                ...
                if data is not None and len(data) > 0:
                    ...
                    if self.bought[symbol] == False and (self.stop_loss[symbol] is None or latest_close > self.stop_loss[symbol]):
                        ...
                        print("Buy:", data[0][1], latest_close, self.stop_loss[symbol])
                    elif self.bought[symbol] == True and self.stop_loss[symbol] is not None:
                        if latest_close <= self.stop_loss[symbol]:
                            quantity = self.portfolio.current_positions[symbol]
                            signal = SignalEvent(symbol, data[0][1], 'EXIT', quantity)
                            self.events.put(signal)
                            self.bought[symbol] = False
                            print("Sell:", data[0][1], latest_close, self.stop_loss[symbol])
                        else:
                            data = self.data.get_latest_data(symbol, N=2)
                            if data is not None and len(data) > 1:
                                if data[1][5] > data[0][5]:
                                    self.stop_loss[symbol] = 0.97 * data[-1][6]
